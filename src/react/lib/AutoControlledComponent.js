 ...
 *
 * 2. Simple Testing
 *    Using a HOC means you must either test the
 *    undecorated component or test through the decorator.
 ...
 *
 * 3. Statics
 *    HOC wrap instances, so statics are no longer accessible.
 *    They can be hoisted, but this is more
 *    looping over properties and storing references.
 *    We rely heavily on statics for testing and sub
 *    components.
 *
 * 4. Instance Methods
...
/**
 * Return the auto controlled state value for a give prop.
 ...
 *  @param {boolean} [includeDefaults=false] Whether or not to
 *                  heed the default props or initial state
 */
...
export default class AutoControlledComponent extends Component {
  constructor(...args) {
    ...
    if (process.env.NODE_ENV !== 'production') {
      ...
      //
      // When setting state, auto controlled props values
      // always win (so the parent can manage them).
      // It is not reasonable to decipher the difference between
      // props from the parent and defaultProps.
